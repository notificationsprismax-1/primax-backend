...
const connUrl = process.env.TRUE_URL || process.env.DATABASE_URL;

if (!connUrl) {
  console.error("ERROR: No database URL found! Set TRUE_URL or DATABASE_URL.");
}

// Parse URL manually to OVERRIDE any ghost PG* env vars Railway injected
const parsed = connUrl ? new URL(connUrl) : null;
const pool = new Pool(parsed ? {
  host: parsed.hostname,
  port: parseInt(parsed.port) || 5432,
  user: parsed.username,
  password: decodeURIComponent(parsed.password),
  database: parsed.pathname.slice(1),
  ssl: parsed.hostname.includes('proxy.rlwy.net') ? { rejectUnauthorized: false } : false
} : {});
